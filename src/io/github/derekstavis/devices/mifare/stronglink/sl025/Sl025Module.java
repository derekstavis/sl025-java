...
public class Sl025Module extends MifareModule {
	...
	private void dispatchResponsePacket(Packet packet) {
		...
		Log.d("Sl025Module: Dispatching a response: " + responsePacket.getCommand().toString());
	...
	public void requestSearch() {
		...
		Log.debugArray(packet.getBytes());
	...
	private void extractPackets() {
		...
		if (buffer.position() > 0) {
			...
			try {
				Log.d("~> Trying to parse a ResponsePacket");
				pkt = ResponsePacket.fromByteBuffer(buffer);
				
				Log.d("pkt = %s\n", pkt);
								
			} catch (IncompletePacketException e) {
				Log.d("Found a incomplete ResponsePacket in buffer");
				
		    } catch (InvalidPacketException e) {
				Log.d("Could not parse a ResponsePacket from buffer");

				Log.d("Discarding a byte from the invalid buffer position.");
				
				buffer.reset();
				buffer.get();
				
				Log.d("Buffer position after discarding: %d\n", buffer.position());
			}
			
			if (pkt != null && pkt.isValid()) {
				
				Log.d("Valid packet enqueued for dispatching.");
				responsePackets.add(pkt);
				
				Log.d("Buffer position after parsing: %d\n", buffer.position());
			}
	...
	private boolean dispatchPackets() {
		...
		if (outputPackets.size() > 0) {
			...
			Log.d("Sl025Module: Dispatching a request: " + cmdPacket.getCommand().toString());
	...
	public void onIOEvent() {
		...
		if (bytes != null && bytes.length > 0) {
			...
			Log.d("Sl025Module: No bytes read!");
		}
